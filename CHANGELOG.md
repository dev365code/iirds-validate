...
than UTF-8 does is refused, and one naming an encoding this reader does not
decode is refused by name.** `metadata.rdf` is read as UTF-8 whatever its
declaration names, so a document declaring `windows-1252` over bytes that are
UTF-8 was read one way here and another by a reader that honours the
declaration -- and passed, where the same declaration over bytes in that code
page was refused with a decode error. The two readings are compared now,
behind a UTF-8 byte order mark as well, and where they differ the document is
refused, naming the declaration (C16.1, S2). For a package with nothing else
wrong, that takes the exit code from `0` to `1` for a false declaration over
UTF-8 text outside ASCII, with or without the mark; for a document all in
ASCII that declares UTF-16, an EBCDIC page such as `cp037`, or an encoding
with escapes or shift states such as `ISO-2022-JP`, `HZ-GB-2312` or
`unicode_escape`; and for a declared name longer than sixty characters, which
no codec's name is, even where Python would read it as one. A single-byte code
...
takes a document declaring one from `0` to `1` on a Windows machine whose
code page is single-byte. The
name is printed with its control characters escaped, quoted where a space
stands at an end, and cut to sixty characters. C16.1's remedy says to change the declaration with the bytes,
...
repeat another, the repeat cannot be counted once, and a whole repeat of such
a structure can be reported as a difference: an L9 finding in such a file whose
difference holds a node without a name says the difference may be that repeat,
not verified. Which structure the node belongs to is not told apart. A finding
