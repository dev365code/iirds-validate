...
[![spec obligations](https://img.shields.io/badge/spec_obligations-170_of_280_covered_·_a_floor-a8721c)](https://github.com/dev365code/iirds-validate/blob/main/docs/requirements.json)
...
<img src="https://raw.githubusercontent.com/dev365code/iirds-validate/main/docs/assets/tenseconds.svg?v=a30efa52" alt="Real iirds check output on a broken package: ERROR M3 metadata declares no iirds:Package for this container, with the fix that follows it; ERROR C5 mimetype must contain exactly application/iirds+zip, with the bytes read from the file and the exact fix; FAIL, 190 rules checked" width="100%">
...
manual.iirds   iiRDS 1.3
  ...
  204 rules checked, 27 not applicable to this version/variant (25 for iiRDS/H, 2 for other editions)
$ echo $?
1
```
...
> **At a glance** — 231 rules across five editions and three profiles · 169 SHACL shapes
...
schema     135/135  the metadata graph  +34 of its own
system     3/3      the run itself  +9 of its own
content    -        iiRDS XHTML5 (Appendix B)  +11 of its own
lint       -        will a consumer be able to use it  +16 of its own
```

157 of 157 catalogued rules, plus 74 of this project's own.
...
**170 of them — a floor, not a ceiling** ([docs/rule-coverage.json](https://github.com/dev365code/iirds-validate/blob/main/docs/rule-coverage.json)),
...
- **Every finding says what to do about it.** All 231 rules carry one imperative
...
- **Every rule has been watched fire.** The suite records which rule ids actually
  produce a finding, and 230 of the 231 have — the remaining one is a `MAY` with
  nothing to violate.
- **What is not established.** The 74 rules this project invented have no second
