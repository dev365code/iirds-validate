...
> **At a glance** · Python 3.9–3.13 · Linux, macOS, Windows · one dependency
> (rdflib), zero for the single-file `.pyz` · 185 rules across five editions
> and three profiles · [what it checks](#what-it-checks) ·
> [limits, measured](#limits-measured) · [how it knows it is right](docs/scope.md) ·
> every number in this file is re-derivable from a committed tool, and the
> sample below is generated by `tests/test_readme_front.py`, which fails when
> it goes stale.
...
manual.iirds   iiRDS 1.3
  ...
  166 rules checked, 19 not applicable to this version/variant
$ echo $?
1
```
