...
59 of the 223 rules have no shape, in four honest categories,
each listed with its reason in `MANIFEST.json`:

- **47 not expressible** — 47 of the 223 rules are about ZIP bytes, content
...
40 of them encode a rule that iiRDS 1.3 added or that only the 1.3 text
...
Every emitted shape is **differentially tested against the 223-rule Python
