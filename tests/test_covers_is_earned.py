...
NAMED_CASES = {
    ...
    "b-6-additional-semantic-tagging-of-content#6":
        "test_appendix_b_claims:test_every_way_of_breaking_an_appendix_b_sentence_is_reported",
    # The sentence's second obligation, and the second row the index makes of
    # it. The named test breaches each limb on its own -- a symbol outside the
    # panel, and two inside one panel -- so the claim on the "only one" half
