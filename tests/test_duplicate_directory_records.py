...
def test_the_rule_says_nothing_about_an_unpacked_container(tmp_path):
    """A directory has no central directory to carry a name twice, and a
    filesystem cannot hold one name twice in one place.

    On this release line the rule guards itself, as S10 and S7 do -- there is
    no archive-only list in the runner to say so in the report, and adding one
    is a change to what a report contains rather than a repair. What can be
    held here is that the rule stands down rather than inventing a finding.
    """
    ...
    assert "S15" not in ids(report), ids(report)
    assert "S14" not in ids(report), ids(report)
