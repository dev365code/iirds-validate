...
def test_the_census_numbers_hold():
    ...
    assert counts["deferred_v1.1"] == 8
    assert counts["not_expressible"] == 43
...
def test_the_shapes_readme_numbers_are_the_manifest_numbers():
    ...
    rules = len(all_rules())
    assert "43 of the %d rules" % rules in readme
    assert str(len(MANIFEST["not_expressible"])) == "43"
    ...
    assert "54 of the %d" % rules in readme
    assert len(MANIFEST["deferred_v1.1"]) == 8
